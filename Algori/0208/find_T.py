...
for tc in range(1, T+1):  # 각 테스트 케이스에 대해 반복합니다.
    ...
    connect = [[] for _ in range(100)]  # 각 노드에 연결된 노드를 저장할 리스트를 초기화합니다. 인접행렬
    for i in range(0, len(arr), 2):  # 입력받은 연결 정보를 통해 그래프를 구성합니다.
        connect[arr[i]].append(arr[i+1]) # 여기까지 데이터 넣는 법
    # 스택 사용
    ...
    while st and nxt != 99:  # 스택이 비어있지 않고, 목표 노드에 도달하지 않았다면 계속 탐색을 진행합니다.
        node = st.pop()  # 스택에서 노드를 하나 꺼내 탐색을 진행합니다.
            for nxt in connect[node]:  # 현재 노드에 연결된 노드의 탐색을 진행합니다. # 만약, 오름 or 내림 차순으로 필요한 문제라면
                # sort or sorted 를 여기 지점에서 해야함
